...
class VoxelRCNNHead(RoIHeadTemplate):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        GRID_SIZE = self.model_cfg.ROI_GRID_POOL.GRID_SIZE #获取网格大小GRID_SIZE=6
        # c_out = sum([x[-1] for x in mlps])
        BIG=GRID_SIZE+2
        SMALL=GRID_SIZE-2
        #pre_channel = GRID_SIZE * GRID_SIZE * GRID_SIZE * c_out
        pre_channel=(BIG**3-SMALL**3)*c_out      
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        for k, src_name in enumerate(self.pool_cfg.FEATURES_SOURCE):
            ...
            pooled_features = pooled_features.view(
                -1, (self.pool_cfg.GRID_SIZE+2) ** 3-(self.pool_cfg.GRID_SIZE-2) ** 3,
    ...
    def get_dense_grid_points(rois, batch_size_rcnn, grid_size):
        """
            根据roi的长宽高，在原基础上，取roi外表面和内表面的两层grid_points来推测如何对roi进行微调
            Args:
            rois:(1024, 7)
            batch_size_rcnn:1024
            grid_size:6
            Returns:
            roi_grid_points: (1024, 8*8*8-4*4*4, 3)
            
        """
        #faked_features = rois.new_ones((grid_size, grid_size, grid_size))
        grid_size_out=grid_size+2      #包裹在roi外表面的grid_size
        grid_size_in=grid_size-2         #roi内部的空心grid_size
        faked_features = rois.new_zeros((grid_size_out, grid_size_out, grid_size_out))
        faked_features[:,:,0]=faked_features[:,:,1]=faked_features[:,:,grid_size_out-2]=faked_features[:,:,grid_size_out-1]=1
        faked_features[:,0,:]=faked_features[:,1,:]=faked_features[:,grid_size_out-2,:]=faked_features[:,grid_size_out-1,:]=1
        faked_features[0,:,:]=faked_features[1,:,:]=faked_features[grid_size_out-2,:,:]=faked_features[grid_size_out-1,:,:]=1
        dense_idx = faked_features.nonzero()  # (N, 3) [x_idx, y_idx, z_idx]
        dense_idx = dense_idx.repeat(batch_size_rcnn, 1, 1).float()  # (B, 448, 3)

        local_roi_size = rois.view(batch_size_rcnn, -1)[:, 3:6]
        roi_grid_points = (dense_idx - 0.5) / grid_size * local_roi_size.unsqueeze(dim=1) \
                          - (local_roi_size.unsqueeze(dim=1) / 2)  # (B, 448, 3)
